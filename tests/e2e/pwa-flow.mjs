...
try{
  ...
  R.head = await ev(`(()=>{const c=document.querySelector('[data-card="1"]'); const sel=c.querySelector('.mask-select'); sel.value="2"; sel.dispatchEvent(new Event("input",{bubbles:true})); sel.dispatchEvent(new Event("change",{bubbles:true})); const c2=document.querySelector('[data-card="1"]'); const sum=c2.querySelector('.summary'); return {title:c2.querySelector('.toggle').innerText, matColor:getComputedStyle(c2.querySelector('.mat')).color, mask:c2.querySelector('.mask-select').value, collapsed:c2.classList.contains("collapsed"), summaryShown:getComputedStyle(sum).display!=="none", summary:sum.innerText, panelMask:[...document.querySelectorAll("#structureBody .stack-layer")].map(b=>b.innerText.split(String.fromCharCode(10)).join(" ")), headOverflow: c2.querySelector('.head').scrollWidth > c2.querySelector('.head').clientWidth}})()`);
