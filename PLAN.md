...
- 3단계 구현 2026-09-17: 폰 기록(새 로그/프리셋에서 시작, 시작·끝 시각, 자동 초안), 업로드(rev 충돌 시 새 파일), 로그 수정, 프리셋 저장·불러오기·삭제, calibration 실측 입력, PC v11 프리셋 저장. 기본 테스트 모드(_mobile_test)로 실기기 확인 대기. 오프라인 업로드 대기열은 4단계
- 4단계 2026-09-17: 오프라인 업로드 대기열(로그·프리셋·실측), Windows 배포판 v11(scripts/build-windows.cjs), 매뉴얼(manual/VTE_Log_Manager_v11_Manual.md), 멤버 안내. 전체 코드 검토 15건 수정. PC v11 편집이 폰 메타·시각·공증착 표시를 보존. 테스트 모드 기본값 꺼짐으로 전환(사용자 승인)
작성: 2026-09-16
